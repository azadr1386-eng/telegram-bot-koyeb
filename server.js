/**
 * Telegram Call Bot (Full Feature) – Part 1
 * -----------------------------------------
 * Features:
 * - Register users with phone code (/register A1234)
 * - Manage contacts (add, list, delete)
 * - Call handling (mention, quick call, buttons)
 * - Call persistence (Supabase or memory)
 * - Forwarding messages during active calls
 * - History of calls
 *
 * Env:
 * BOT_TOKEN
...
const BASE_URL = process.env.BASE_URL || "https://telegram-bot-koyeb-19.onrender.com";
...
if (!BASE_URL) {
  console.error("❌ BASE_URL تنظیم نشده است");
  process.exit(1);
}
...
if (process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY) {
  ...
  console.warn("⚠️ Supabase تنظیم نشده — حافظه محلی استفاده می‌شود.");
}

// --------- MEMORY FALLBACK ---------
global.users = global.users || {}; // user_id -> { phone_number, username, group_id }
global.contacts = global.contacts || {}; // user_id -> [ { contact_name, phone_number } ]
global.activeCalls = global.activeCalls || {}; // callId -> callData
global.callHistory = global.callHistory || []; // array of calls
...
function createContactButtons(contacts) {
  ...
  for (let i = 0; i < contacts.length; i += 3) {
    buttons.push(
      contacts.slice(i, i + 3).map((c) =>
        Markup.button.callback(`👤 ${c.contact_name}`, `quick_call_${c.phone_number}`)
      )
    );
  }
...
async function findUserByPhone(phone) {
  ...
  if (supabase) {
    ...
    for (const [uid, u] of Object.entries(global.users)) {
      if ((u.phone_number || "").toUpperCase() === target)
        return { ...u, user_id: Number(uid) };
    }
    return null;
  }
}
...
async function userHasActiveCall(userId) {
  for (const c of Object.values(global.activeCalls || {})) {
    if (
      ...
      ["ringing", "answered"].includes(c.status)
    )
      return true;
  }
  return false;
...
bot.command("register", async (ctx) => {
  if (ctx.chat.type === "private")
    return ctx.reply("❌ فقط در گروه‌ها استفاده شود.");
...
bot.action(/reject_call_(.+)/, async (ctx) => {
  ...
  try {
    if (call.callerChatId && call.callerMessageId) await bot.telegram.editMessageText(call.callerChatId, call.callerMessageId, null, "❌ تماس رد شد.");
  } catch (e) {}
  try {
    if (call.receiverChatId && call.receiverMessageId) await bot.telegram.editMessageText(call.receiverChatId, call.receiverMessageId, null, "❌ تماس رد شد.");
  } catch (e) {}
...
bot.action(/end_call_(.+)/, async (ctx) => {
  ...
  try {
    if (call.callerChatId && call.callerMessageId) await bot.telegram.editMessageText(call.callerChatId, call.callerMessageId, null, `⏹️ پایان یافت\n⏱ ${call.duration}s`);
  } catch (e) {}
  try {
    if (call.receiverChatId && call.receiverMessageId) await bot.telegram.editMessageText(call.receiverChatId, call.receiverMessageId, null, `⏹️ پایان یافت\n⏱ ${call.duration}s`);
  } catch (e) {}
...
bot.on('message', async ctx => {
  ...
  if (ctx.message.reply_to_message) {
    ...
    // Check if it's a forwarded call message
    for (const call of Object.values(global.activeCalls)) {
      if (
        (replyTo.message_id === call.callerMessageId && ctx.chat.id === call.callerChatId) ||
        (replyTo.message_id === call.receiverMessageId && ctx.chat.id === call.receiverChatId)
      ) {
        const targetChat = ctx.chat.id === call.callerChatId ? call.receiverChatId : call.callerChatId;
        await bot.telegram.sendMessage(targetChat, `💬 پاسخ از ${ctx.from.first_name}:\n${ctx.message.text}`, {
          reply_to_message_id: ctx.chat.id === call.callerChatId ? call.receiverMessageId : call.callerMessageId,
        });
      }
    }
  }
...
// ---------- contacts callback handlers ----------
bot.action('add_contact', ctx => {
  ctx.session.userState = USER_STATES.AWAITING_CONTACT_NAME;
  ctx.reply('نام مخاطب را وارد کنید:');
});
bot.action('delete_contact', async ctx => {
  let contacts = supabase
    ? (await supabase.from('contacts').select('*').eq('user_id', ctx.from.id)).data
    : global.contacts[ctx.from.id] || [];
  if (!contacts.length) return ctx.reply('❌ مخاطبی برای حذف وجود ندارد.');
  const buttons = contacts.map(c =>
    [Markup.button.callback(`🗑️ ${c.contact_name}`, `delete_${c.phone_number}`)]
  );
  buttons.push([Markup.button.callback('🔙 بازگشت', 'back_to_main')]);
  ctx.reply('مخاطب را انتخاب کنید:', Markup.inlineKeyboard(buttons));
});
bot.action(/delete_(.+)/, async ctx => {
  const phone = ctx.match[1];
  if (supabase) await supabase.from('contacts').delete().eq('user_id', ctx.from.id).eq('phone_number', phone);
  else global.contacts[ctx.from.id] = (global.contacts[ctx.from.id] || []).filter(c => c.phone_number !== phone);
  ctx.answerCbQuery('✅ مخاطب حذف شد.');
  ctx.editMessageText('✅ مخاطب حذف شد.');
});
...
bot.action(/quick_call_(.+)/, async ctx => {
  ...
  if (await userHasActiveCall(ctx.from.id)) return ctx.answerCbQuery('❌ شما در تماس هستید.');
  if (await userHasActiveCall(targetUser.user_id)) return ctx.answerCbQuery('❌ کاربر مقصد در تماس است.');
  ...
  await persistActiveCall(callData);
  setTimeout(() => autoMissCall(callId).catch(console.error), 60000);
  ctx.answerCbQuery('✅ تماس شروع شد.');
});

// ---------- back button ----------
bot.action('back_to_main', ctx => {
  ctx.editMessageText('🔙 منو اصلی:', createMainMenu());
});
